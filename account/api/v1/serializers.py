...
class UserFollowerSerializer(serializers.ModelSerializer):
    class Meta:
        model = UserFollowing
        fields = ['date_created', 'following_user']
    ...
    def create(self, validated_data):
        ...
        user = UserFollowing.objects.create(
            user=user, following_user=validated_data.get('following_user'))
        return user
    ...
    def validate(self, attrs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if UserFollowing.objects.filter(user=user, following_user=following_user):
            raise ValidationError(
                {'following_user': 'User is already being followed'})

        return attrs

    def to_representation(self, instance):
        return {
            'user': UserSerializer(instance=instance.following_user).data,
