...
from account.api.v1.serializers import NotificationSerializer
from tests.utils.TestCase import SerializerTestCase
from tests.v1.account.test_models import NotificationFactory, UserFactory
...
class LikeSerializerTest(SerializerTestCase):
    def setUp(self) -> None:
        ...
        self.playlist = PlayListFactory.create(created_by=user_2, title="Valkrie")
        playlist_2 = PlayListFactory.create(created_by=self.user, title="Magnum")
        liked_playlist = PlayListFactory.create(created_by=user_2, title="Valdor")
        ...
        self.INVALID_DATA = [
            {
                ...
                "errors": {"songs": ["This list may not be empty."]},
            ...
            },
            {
                ...
                "errors": {"title": ["PlayList with this title already exists."]},
...
class CommentSerializerTests(SerializerTestCase):
    ...
    def test_non_required_fields(self) -> None:
        self.check_non_required_fields(self.serializer, self.NON_REQUIREDS_FIELDS)
...
class NotificationSerializerTests(SerializerTestCase):
    def setUp(self) -> None:
